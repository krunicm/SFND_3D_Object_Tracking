...
int main(int argc, const char *argv[])
{
    ...
    cout << "Combinations Detector/Descriptor \n\n" << endl;

    int counter = 0;
    for (string detectorType : detectorsList)
    {
        for (string descriptorType : descriptorsList)
        {   
            counter++;
            string header = "\n### " + std::to_string(counter) + ". Combo **" + detectorType + "/" + descriptorType + "**\n\n";
                ...
                detectorType.compare("AKAZE") != 0 && descriptorType.compare("AKAZE") == 0 )
            {
                ...
                cout << "| TTC Lidar (s) | TTC Camera (s) | Difference (s) |" << endl;
                cout << "| -----------   | ----------- | ----------- |" << endl;
            ...
            for (size_t imgIndex = 0; imgIndex <= imgEndIndex - imgStartIndex; imgIndex+=imgStepWidth)
            {
                ...
                if(bVis)
                {
                    ...
                    showLidarTopview((dataBuffer.end() - 1)->lidarPoints);
                }
                bVis = false;
                ...
                if (dataBuffer.size() > 1) // wait until at least two images have been processed
                {
                    ...
                    for (auto it1 = (dataBuffer.end() - 1)->bbMatches.begin(); it1 != (dataBuffer.end() - 1)->bbMatches.end(); ++it1)
                    {
                        ...
                        if( currBB->lidarPoints.size()>0 && prevBB->lidarPoints.size()>0 ) // only compute TTC if we have Lidar points
                        {
                            ...
                            cout << "| " << ttcLidar << " | " << ttcCamera << " | " << abs(ttcLidar - ttcCamera) << " | " << endl;
                            diff.push_back(abs(ttcLidar - ttcCamera));
